...
```{r assoc_current}
assoc_current_wide <- read_excel("raw-data/Faculty affiliations.xlsx") %>%
...
new_assoc_current_wide <- read_excel("raw-data/Faculty affiliations.xlsx", sheet = 2) %>%
  pivot_longer(-faculty.member) %>%
...
again_new_assoc_current_wide <- read_excel("raw-data/Faculty affiliations.xlsx", sheet = 3) %>%
  pivot_longer(-faculty.member) %>%
  replace_na(list(value = 0)) %>%
  mutate(value = as.integer(value)) %>%
  pivot_wider(names_from = name, values_from = value)
...
### (Yet another) Proposed program structure

This plot represents yet antoerh proposed program structure, with my faculty assignments from the sheet above.
